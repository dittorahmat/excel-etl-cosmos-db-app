...
export function safeCompareKeys(key1: string, key2: string): boolean {
  // If either key is not a string, return false
  if (typeof key1 !== 'string' || typeof key2 !== 'string') {
    return false;
  }

  // If either key is not a valid hex string, return false
  const hexRegex = /^[0-9a-fA-F]+$/;
  if (!hexRegex.test(key1) || !hexRegex.test(key2)) {
    return false;
  }

  // If the hex strings have different lengths, they can't be equal
