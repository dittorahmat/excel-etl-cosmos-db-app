...
const ComplexComponent = () => {
    ...
    const handleUpdate = useCallback(async (id, newValue) => {
        try {
    ...
    return (_jsxs("div", { children: [_jsx("h1", { "data-testid": "title", children: "Complex Component" }), _jsxs("div", { "data-testid": "counter", children: ["Counter: ", counter] }), _jsx("div", { "data-testid": "items-list", children: items.map(item => (_jsxs("div", { "data-testid": `item-${item.id}`, children: [_jsx("span", { children: item.name }), _jsx("span", { "data-testid": `value-${item.id}`, children: item.value.toFixed(2) }), _jsx("button", { "data-testid": `select-${item.id}`, onClick: () => handleSelect(item.id), children: selectedId === item.id ? 'Deselect' : 'Select' }), selectedId === item.id && (_jsxs("div", { "data-testid": `actions-${item.id}`, children: [_jsx("button", { "data-testid": `update-${item.id}`, onClick: () => handleUpdate(item.id, Math.random() * 100), children: "Update" }), _jsx("button", { "data-testid": `delete-${item.id}`, onClick: () => handleDelete(item.id), children: "Delete" })] }))] }, item.id))) })] }));
};
