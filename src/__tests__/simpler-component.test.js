...
const SimpleList = () => {
    ...
    const handleUpdate = async (id) => {
        ...
        setItems(prev => prev.map(item => item.id === id ? { ...item, value: Math.random() * 100 } : item));
    };
    if (loading)
        return _jsx("div", { "data-testid": "loading", children: "Loading..." });
    return (_jsxs("div", { children: [_jsx("h1", { "data-testid": "title", children: "Simple List" }), _jsx("ul", { "data-testid": "items-list", children: items.map(item => (_jsxs("li", { "data-testid": `item-${item.id}`, children: [item.name, ": ", item.value.toFixed(2), _jsx("button", { "data-testid": `update-${item.id}`, onClick: () => handleUpdate(item.id), children: "Update" })] }, item.id))) })] }));
};
