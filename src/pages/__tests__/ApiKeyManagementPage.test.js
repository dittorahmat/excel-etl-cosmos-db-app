...
vi.mock('../../components/ApiQueryBuilder/ApiQueryBuilder', () => ({
    ApiQueryBuilder: () => _jsx("div", { children: "ApiQueryBuilder Mock" }),
}));
// Mock the toast hook
...
vi.mock('date-fns', async (importOriginal) => {
    ...
    return {
        ...
        format: vi.fn((date, fmt) => {
            ...
            if (fmt === 'PPP')
                return `FormattedDateShort-${date.toISOString()}`;
...
describe('ApiKeyManagementPage', () => {
    ...
    const renderComponent = async () => {
        ...
        const { container } = render(_jsx(MemoryRouter, { children: _jsx(ApiKeyManagementPage, {}) }));
